...
#
#
#     punctuations.append("<REL>")
#     ''' start '''
#     bpe_tokens_a = tokenizer.tokenize(text_a)
#     bpe_tokens_b = tokenizer.tokenize(text_b)
#     bpe_tokens = [tokenizer.bos_token]+['<SPL>']+["<REL>"] + bpe_tokens_a + [tokenizer.sep_token]*2 + \
#                  bpe_tokens_b + [tokenizer.eos_token]
#     #begin + text_a + sep + text_b + end  token list中元素均为数字
#     a_mask = [1] * (len(bpe_tokens_a) + 4) + [0] * (max_length - (len(bpe_tokens_a) + 2))
#     b_mask = [0] * (len(bpe_tokens_a) + 4) + [1] * (len(bpe_tokens_b) + 2) + [0] * (max_length - len(bpe_tokens))
...
#     segment_ids = [0] * (len(bpe_tokens_a) + 3) + [1] * (len(bpe_tokens_b) + 1)
#
...
def arg_tokenizer(text_a, text_b, tokenizer, stopwords, relations:dict, punctuations:list,
    ...
    bpe_tokens = [tokenizer.bos_token] + bpe_tokens_a + [tokenizer.sep_token] + \
                 bpe_tokens_b + [tokenizer.eos_token]
    #begin + text_a + sep + text_b + end  token list中元素均为数字
    a_mask = [1] * (len(bpe_tokens_a) + 1) + [0] * (max_length - (len(bpe_tokens_a) + 1))
    b_mask = [0] * (len(bpe_tokens_a) + 1) + [1] * (len(bpe_tokens_b) + 2) + [0] * (max_length - len(bpe_tokens))
    a_mask = a_mask[:max_length]
    b_mask = b_mask[:max_length]
    assert len(a_mask) == max_length, 'len_a_mask={}, max_len={}'.format(len(a_mask), max_length)
    assert len(b_mask) == max_length, 'len_b_mask={}, max_len={}'.format(len(b_mask), max_length)
