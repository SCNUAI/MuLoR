...
def load_and_cache_examples(args,tokenizer,arg_tokenizer, evaluate=False, test=False):
    ...
    dataset = TensorDataset(all_input_ids, all_attention_mask,all_passage_mask, all_question_mask,all_argument_bpe_ids,all_domain_bpe_ids,
                            all_punct_bpe_ids,all_keywordids,all_keymask,all_SVO_ids,all_SVO_mask,all_key_segid, all_label_ids, all_adj_SVO)
    return dataset
...
def SVOlist(svo_token_ids,tokenizer):
    ...
    # print('svo_token_ids',svo_token_ids)
    # print('svo_set',svo_set)
...
class InputFeatures(object):  #构建一个样例 包含1context+4answers
    def __init__(self, example_id, input_ids, attention_mask,passage_mask,question_mask,
                 ...
                 SVO_ids,SVO_mask,key_segid,adj_of_choice,label):
        self.example_id = example_id
...
class DataProcessor(object):
    ...
    def get_labels(self):
        ...
        raise NotImplementedError()
class ExlogicProcessor(DataProcessor):
    """Processor for the Exlogic data set."""

    def get_train_examples(self, data_dir):
        """See base class."""
        logger.info("LOOKING AT {} train".format(data_dir))
        return self._create_examples(self._read_json(os.path.join(data_dir, "train_en.json")), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        logger.info("LOOKING AT {} dev".format(data_dir))
        return self._create_examples(self._read_json(os.path.join(data_dir, "val_en.json")), "dev")

    def get_test_examples(self, data_dir):
        logger.info("LOOKING AT {} test".format(data_dir))
        return self._create_examples(self._read_json(os.path.join(data_dir, "test_en.json")), "test")
        ...
        for d in lines:
            ...
            examples.append(
                InputExample(
                    ...
                    endings=[choice_A, choice_B, choice_C, choice_D],
                    label=label,
                )
            )
            # else:
...
class LogiQAProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, type):
        ...
        for i in range(n_examples):
            ...
            if context[-1] not in punctuations:
                context = context + '.'
...
class LogiProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, type):
        ...
        for d in lines:
            ...
            question = d['question']
            answers = d['options']
            label = d['answer'] # for test set, there is no label. Just use 0 for convenience.
            id_string = int(d['id'])
            examples.append(
                InputExample(
                    example_id = id_string,
...
processors = {"reclor": ReclorProcessor, "logiqa": LogiQAProcessor,"exlogic": ExlogicProcessor,"logiqas": LogiProcessor}
MULTIPLE_CHOICE_TASKS_NUM_LABELS = {"race": 4, "swag": 4, "arc": 4, "reclor": 4,"logiqa":4,"logiqas":4}
